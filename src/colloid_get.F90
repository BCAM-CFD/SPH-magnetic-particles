...
9999    CONTINUE
      ...
      SUBROUTINE colloid_get_magnet_mom(this,d_magnet_mom,stat_info)
        !---------------------------------------
        ! Return torque of colloids.
        !---------------------------------------
        
        TYPE(Colloid), INTENT(IN)               :: this
        REAL(MK), DIMENSION(:,:), POINTER       :: d_magnet_mom
        INTEGER, INTENT(OUT)                    :: stat_info
        
        
        stat_info = 0
        
        IF(ASSOCIATED(d_magnet_mom)) THEN           
           DEALLOCATE(d_magnet_mom)
        END IF
        
        ALLOCATE(d_magnet_mom(3,this%num_colloid))
        
        d_magnet_mom(1:3,1:this%num_colloid) = &
             this%magnet_mom(1:3,1:this%num_colloid)
        
        RETURN       
        
      END SUBROUTINE colloid_get_magnet_mom
      !***************************************************
