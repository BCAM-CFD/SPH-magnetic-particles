      ...
      SUBROUTINE colloid_compute_interaction_implicit_velocity_pair(this,&
           comm,rank, MPI_PREC,step,dt,drag,torque,FB,stat_info)
        !----------------------------------------------------
        ...
        REAL(MK),DIMENSION(3)                   :: T_ij, T_ji
        REAL(MK), DIMENSION(:,:), ALLOCATABLE   :: magnetic_torque
        !************************************
	...
	!****************************************
        ...
        IF ( this%cc_lub_type > mcf_cc_lub_type_no ) THEN
           ...
           IF ( sweep_adaptive ) THEN
              ...
              !**** Added by Adolfo for the magnetic alternating field *****
              ALLOCATE(magnetic_torque(dim,num))
              !**************************************************************
        ...
        IF ( this%cc_repul_type > mcf_cc_repul_type_no .OR. &
           ...
           !***** Added by Adolfo (magnetic) ************
           magnetic_torque(:,:) = 0.0_MK
           !*********************************************
           ...
           DO sub_step = 1, num_sub_step
              ...
              !************** Added by Adolfo (magnetic) ***************
              !----------------------------------------------
              ! Magnetic torque and force are treated as the repulsion force
              !----------------------------------------------
              DO I = 1, num - 1
                 DO J = I + 1, num
                    IF ( this%cc_magnet_type == mcf_cc_magnet_type_field ) THEN
                       ...
                       IF ( r <= this%cc_magnet_cut_off ) THEN
                          
                          IF ( r <= this%cc_magnet_cut_on ) THEN
                             
                             r = this%cc_magnet_cut_on
                             
                          END IF
                          
                          eij(1:dim) = rij(1:dim) / r
                          CALL colloid_compute_magnetism_cc(this,&
                               eij, r, i, j, F_ij(1:dim), T_ij(1:dim), T_ji(1:dim), stat_info_sub)

                          !-- F_ij is the magnetic force exerted by j on i --
                          F_repul(1:dim,i) = F_repul(1:dim,i) - F_ij(1:dim)
                          F_repul(1:dim,j) = F_repul(1:dim,j) + F_ij(1:dim)

                          !-- T_ij is the torque on j by i
                          !-- T_ji is the torque on i by j                          
                          magnetic_torque(1:dim,i)  = magnetic_torque(1:dim,i)  + T_ji(1:dim)
                          magnetic_torque(1:dim,j)  = magnetic_torque(1:dim,j)  + T_ij(1:dim)
              ...
              DO i = 1, num
                 ...
                 !***** Added by Adolfo (magnetic) ************
                 this%omega(1:dim,i,1) = &
                      this%omega(1:dim,i,1) + &
                      0.5_MK *  magnetic_torque(1:dim,i) * dt_sub / this%mmi(1:dim,i)                 
                 !*********************************************
              ...
              !************** Added by Adolfo (magnetic) ***************
              !----------------------------------------------
              ! Magnetic force and torque are calculated again with new positions
              !----------------------------------------------
                 ...
                 DO J = I + 1, num
                    IF ( this%cc_magnet_type == mcf_cc_magnet_type_field ) THEN
                       ...
                       IF ( r <= this%cc_magnet_cut_off ) THEN
                          ...
                          !-- F_ij is the force exerted by j on i --
                          F_repul(1:dim,i) = F_repul(1:dim,i) - F_ij(1:dim)
                          F_repul(1:dim,j) = F_repul(1:dim,j) + F_ij(1:dim)
...
9999    CONTINUE
        ...
        !**** Added by Adolfo ********
        IF (ALLOCATED(v_backup)) THEN
           DEALLOCATE(v_backup)
        ENDIF
        IF (ALLOCATED(v_sph)) THEN
           DEALLOCATE(v_sph)
        ENDIF
        IF (ALLOCATED(v_update)) THEN
           DEALLOCATE(v_update)
        ENDIF
        IF (ALLOCATED(v_diff)) THEN
           DEALLOCATE(v_diff)
        ENDIF
        IF (ALLOCATED(v_normal)) THEN
           DEALLOCATE(v_normal)
        ENDIF
        !****************************

        !********* Added by Adolfo (magnetic) *********
        IF (ALLOCATED(magnetic_torque)) THEN
           DEALLOCATE(magnetic_torque)
        ENDIF
        !**********************************************
