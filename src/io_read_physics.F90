...
!        REAL(MK)                                :: cc_magnet_mu
        ...
        !********* Added by Adolfo (magnet) **************
        REAL(MK),ALLOCATABLE,DIMENSION(:,:)     :: coll_magnet_mom
        INTEGER                                 :: magnet_mom_index
        REAL(MK)                                :: magnetic_B_amp
        !*************************************************
	...
	!----------------------------------------------------
        ...
        iline = 0
          ...
          ELSE IF (carg =='KSAI') THEN	  
             ...
             !******** Added by Adolfo ****************
          ELSE IF (carg =='MAGNETIC_B_AMP') THEN

             !-----------------------------------------------
             ! Shear viscosity
             !-----------------------------------------------
             
             READ(cvalue,*,IOSTAT=ios, ERR=200) magnetic_B_amp

             CALL physics_set_magnetic_B_amp(phys,magnetic_B_amp,stat_info_sub)
             !******************************************
...
!             cc_magnet_mu      = 1.0
             ...
             ALLOCATE(coll_magnet_mom(num_dim,num_colloid))
             !***********************************************
...
!!$             READ(cvalue,*,IOSTAT=ios,ERR=200) cc_magnet_mu
          ...
          ELSE IF (carg == 'COLL_V' .AND. &
             ...
             !********** Added by Adolfo (magnetic) ************
          ELSE IF (carg == 'COLL_MAGNET_MOM' .AND. &
               num_species > 1 .AND. &
               num_colloid > 0 .AND. &
               magnet_mom_index >= 0 .AND.  &
               magnet_mom_index <num_colloid ) THEN
             
             magnet_mom_index = magnet_mom_index + 1
             
             READ(cvalue,*,IOSTAT=ios, ERR=200)  &
                  coll_magnet_mom(1:num_dim,magnet_mom_index)
             !***************************************************
...
!!$                      cc_magnet_mu,stat_info_sub)
              ...
              CALL colloid_set_v(colloids,&
                   ...
                   stat_info_sub)
              !********* Added by Adolfo (magnetic) ************
              !--- Magnet moment and its amplitude are stored ---
              CALL colloid_set_magnet_mom(colloids,&
                   coll_magnet_mom(1:num_dim,1:num_colloid),stat_info_sub)
              !**************************************************
	...
	!----------------------------------------------------
        ...
        !********** Added by Adolfo (magnetic) *************
        IF(ALLOCATED(coll_magnet_mom))THEN
           DEALLOCATE(coll_magnet_mom)
        END IF
        !***************************************************
