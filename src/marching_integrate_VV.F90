	...
	!********************************
        ...
        !********* Added by Adolfo (magnetic) ************
        magnetic_B_amp = physics_get_magnetic_B_amp(this%phys,stat_info_sub)
        !*************************************************
...
!	   !****************************
           ...
           IF ( coll_translate .OR. coll_rotate ) THEN
              ...
              DO i = 1, coll_sub_time_step
                 ...
                 !************* Added by Adolfo (magnetic) ******************
                 ! Torque due to misalignment between the magnetic moment and the magnetic field.
                 CALL colloid_compute_magnetic_torque(colloids, time, magnetic_B_amp, &
                      coll_torque, stat_info_sub)
                 !***********************************************************
                 ...
                 CASE (1:2)
                    ...
                    !******* Comment by Adolfo (magnetic): in the following subroutine the 
                    !        magnetic dipole-dipole force are included,
                    !        but this is not the semi-implicit case. *************
...
!!$                         wall_drag_c(1:num_dim,1:num_dim*2),stat_info_sub)
                    ...
                    !******* Comment by Adolfo (magnetic): in the following subroutine the 
                    !        magnetic dipole-dipole interactions are included *************
                    CALL colloid_compute_interaction_implicit_velocity_pair(&
                         colloids, comm, rank, MPI_PREC, step, dt_sub_time_step,&
                         coll_drag,coll_torque, &
                         wall_drag_c(1:num_dim,1:num_dim*2),stat_info_sub)
!**************************************
...
!                            colloid_get_implicit_pair_sweep_error_tang(colloids,stat_info_sub)
		!**************************************
                 ...
                 CALL colloid_compute_magnetic_moment(colloids, dt, stat_info_sub)           
                 !***************************************
