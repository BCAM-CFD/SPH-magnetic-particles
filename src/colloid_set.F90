...
!******************* Added by Adolfo (magnetic) *********************
      SUBROUTINE colloid_set_cc_magnet_B0(this,d_cc_magnet_B0,stat_info)
        !----------------------------------------------------
        ! Set magnetic permittivity.
        !----------------------------------------------------
        
        TYPE(Colloid), INTENT(INOUT)    :: this
        REAL(MK), INTENT(IN)            :: d_cc_magnet_B0
        INTEGER, INTENT(OUT)            :: stat_info
        
        stat_info = 0
        
        this%cc_magnet_B0 = d_cc_magnet_B0
        
        RETURN
        
      END SUBROUTINE colloid_set_cc_magnet_B0

      SUBROUTINE colloid_set_cc_magnet_m0(this,d_cc_magnet_m0,stat_info)
        !----------------------------------------------------
        ! Set magnetic permittivity.
        !----------------------------------------------------
        
        TYPE(Colloid), INTENT(INOUT)    :: this
        REAL(MK), INTENT(IN)            :: d_cc_magnet_m0
        INTEGER, INTENT(OUT)            :: stat_info
        
        stat_info = 0
        
        this%cc_magnet_m0 = d_cc_magnet_m0
        
        RETURN
        
      END SUBROUTINE colloid_set_cc_magnet_m0
...
9999    CONTINUE
      ...
      !************ Added by Adolfo (magnetic) **************
      SUBROUTINE colloid_set_magnet_mom(this,d_magnet_mom,stat_info)
        !----------------------------------------------------
        ! Set the magnetic moment of colloids and its magnitude.
        !----------------------------------------------------
        
        TYPE(Colloid), INTENT(INOUT)            :: this
        REAL(MK), DIMENSION(:,:), INTENT(IN)    :: d_magnet_mom
        INTEGER, INTENT(OUT)                    :: stat_info
        ...
        ! and num of colloids match.
        !---------------------------------------------------
        dim = SIZE(d_magnet_mom,1)
        num = SIZE(d_magnet_mom,2)
        
        IF( dim /= this%num_dim) THEN
           PRINT *, "colloid_set_magnet_mom: ", "Wrong Dimension !"
           stat_info = -1
           GOTO 9999
        END IF
        
        IF( num /= this%num_colloid) THEN
           PRINT *, "colloid_set_magnet_mom: ", "Wrong number of colloids !"
           stat_info = -1
           GOTO 9999
        END IF
        
        this%magnet_mom(:,:) = d_magnet_mom(:,:)
        DO I = 1, num
           this%magnet_mom_magnitude(I) = 0.0_MK
           DO J = 1, dim
              this%magnet_mom_magnitude(I) = this%magnet_mom_magnitude(I) + this%magnet_mom(J,I)**2.0
           ENDDO
           this%magnet_mom_magnitude(I) = SQRT(this%magnet_mom_magnitude(I))
        ENDDO
...
9999    CONTINUE
      ...
      END SUBROUTINE colloid_set_magnet_mom
      !*****************************************************
